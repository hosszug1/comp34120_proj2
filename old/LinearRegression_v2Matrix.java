public class LinearRegression 
{
	...
	public double[] gradientDescent(double[][] x, double[] y, double[] theta, double learningRate, int numberOfIterations)
	{
		...
		{
			// hypothesis = MatrixT.multiply(x, theta);
			double[][] newTheta = new double[theta.length][1];
			for (int j = 0; j < theta.length; j++)
				newTheta[j][0] = theta[j];
			Matrix thetaMatrix = new Matrix(newTheta);
			Matrix hypothesisMatrix = xMatrix.times(thetaMatrix);

			// diffHY = MatrixT.subtract(hypothesis, y);
			double[][] newY = new double[y.length][1];
			for (int j = 0; j < y.length; j++)
				newY[j][0] = y[j];
			Matrix yMatrix = new Matrix(newY);
			Matrix diffHYMatrix = hypothesisMatrix.minus(yMatrix);

			// xTransXdiffHY = MatrixT.multiply(xTranspose, diffHY);
			Matrix xTransXdiffHYMatrix = xMatrixTranspose.times(diffHYMatrix);

			// newThetas = MatrixT.subtract(newThetas, MatrixT.multiply(xTransXdiffHY, learningRate * (1.0 / m)));
			Matrix newThetasMatrix = thetaMatrix.minus(xTransXdiffHYMatrix.times(learningRate * (1.0 / m)));
			for (int j = 0; j < theta.length; j++)
				newThetas[j] = newThetasMatrix.data[j][0];
		} // for
